...
function add_quantity(obj,rec_id){
    var item = $('#input_item_' + rec_id);
    var orig = Number(item.val());
    item.val(orig + 1);
    item.keyup();
	
	alert($(obj).parents().children(".price").text()+'....');
	alert($(obj).parents().children(".subtotal").text('testing....'));

	/*
    var prices = item.parents().children('.price').text().substr(1);  // 单价
    item.parent().next().text(prices*(orig + 1));   // 小计
    
    //$(".subtotal").each(function(i,o){
    $("dd[class='subtotal fs14 strong']").each(function(i,o){
