    ...
    <div class="wrap">
      <div class="public">
        <div class="user_search">
          <form method="get" action="<?php echo U('MemOrder/Mem_order_list');?>">
            <span>下单时间:</span>
            <input type="text" class="text1 width2" name="add_time_from" id="add_time_from" value="<?php echo ($add_time_from); ?>"/>
            &#8211;
            <input type="text" class="text1 width2" name="add_time_to" id="add_time_to" value="<?php echo ($add_time_to); ?>"/>
            <span>订单号:</span>
            <input type="text" class="text1 width_normal" name="order_sn" value="<?php echo ($order_sn); ?>">
            <span>订单状态</span>
            <select name="status">
              <option  value="">所有订单</option>
              <option <?php if(($status) == "0"): ?>selected<?php endif; ?> value="0">交易关闭</option>
              <option <?php if(($status) == "11"): ?>selected<?php endif; ?> value="11">等待买家付款</option>
              <option <?php if(($status) == "20"): ?>selected<?php endif; ?> value="20">买家已付款</option>
              <option <?php if(($status) == "30"): ?>selected<?php endif; ?> value="30">已发货</option>
              <option <?php if(($status) == "40"): ?>selected<?php endif; ?> value="40">交易已完成</option>
        ...
        <?php if(is_array($valarr)): $i = 0; $__LIST__ = $valarr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="order_form">
          <h2>
		...
		<!-- 遍历每个订单中的商品 -->
		<?php if(is_array($arr[$v['order_id']])): $i = 0; $__LIST__ = $arr[$v['order_id']];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$x): $mod = ($i % 2 );++$i;?><div class="con">
            <p class="ware_pic">
		...
		<!-- 遍历结束 -->
          ...
          <div class="foot">
            ...
            <div class="handle">
              ...
              <?php if(($v['status']) == "40"): ?><a class="btn1" href="index.php?app=buyer_order&amp;act=evaluate&amp;order_id=258" target="_blank" id="order258_evaluate" style="display:none">我要评价</a><?php endif; ?>
              <?php if(($v['status']) == "11"): ?><a href="index.php?app=cashier&amp;order_id=258" target="_blank" id="order258_action_pay" class="btn">付款</a><?php endif; ?>
              <?php if(($v['status']) == "30"): ?><input type="button" value="确认收货" class="btn1" ectype="dialog" dialog_id="buyer_order_confirm_order" dialog_width="400" dialog_title="确认收货" uri="index.php?app=buyer_order&amp;act=confirm_order&order_id=258&ajax"  id="order258_action_confirm" style="display:none" /><?php endif; ?>
              <?php if(($v['status']) == "11"): ?><input type="button" value="取消订单" class="btn1" ectype="dialog" dialog_width="400" dialog_title="取消订单" dialog_id="buyer_order_cancel_order" uri="member_cancel_order.html"  id="order258_action_cancel" onclick="cancelorder(<?php echo ($v['order_id']); ?>,<?php echo ($p); ?>);" /><?php endif; ?>
              <a href='<?php echo U("Home/MemOrder/Mem_order_show/id/{$v['order_id']}/p/{$p}");?>' class="btn1">查看订单</a>
            </div>
          </div>
        </div><?php endforeach; endif; else: echo "" ;endif; ?>
   

        <div class="pagestr"><?php echo ($pageStr); ?></div>
...
<script>
	function cancelorder(id,p){
		if(confirm("确定要取消吗?")){
			$.post("<?php echo U('MemOrder/Mem_order_cancel');?>",{"id":id},function(d){
				alert(d);
				location.href="<?php echo U('Home/MemOrder/Mem_order_list/p/"+p+"');?>";
			})
		}
...
.pagestr a.first,.pages a.prev,.pages a.next,.pages a.end{
    margin:0;
}
.pagestr a:hover{
    border-color:#50A8E6;
